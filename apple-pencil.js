// Apple Pencil Settings JavaScript
class ApplePencilSettings {
    constructor() {
    ...
    saveSetting(toggle) {
        ...
        localStorage.setItem(`apple_pencil_${label.replace(/\s+/g, '_').toLowerCase()}`, isActive);
        console.log(`Apple Pencil ${label}: ${isActive ? 'enabled' : 'disabled'}`);
    }
    ...
    loadSettings() {
        this.settingRows.forEach(row => {
            ...
            if (toggle) {
                const label = row.querySelector('.setting-label').textContent;
                const saved = localStorage.getItem(`apple_pencil_${label.replace(/\s+/g, '_').toLowerCase()}`);
